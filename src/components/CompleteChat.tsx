...
const CompleteChat: React.FC<CompleteChatProps> = ({
  ...
  const simulateAssistantResponse = useCallback(
    (userMessage: string) => {
      ...
      setTimeout(
        () => {
          ...
          if (
            ...
            response =
              '¡Por supuesto! Puedo ayudarte con la planificación de estudios, crear horarios personalizados y darte consejos para mejorar tu rendimiento académico. ¿Qué necesitas específicamente?';
          } else if (
            lowerMessage.includes('matematicas') ||
            lowerMessage.includes('matemáticas')
          ) {
            response =
              'Matemáticas es una materia que requiere práctica constante. Te recomiendo estudiar 30 minutos diarios, resolver ejercicios variados y revisar conceptos básicos regularmente. ¿En qué tema específico necesitas ayuda?';
          } else if (
            lowerMessage.includes('examen') ||
            lowerMessage.includes('examenes')
          ) {
            response =
              'Para prepararte para exámenes, te sugiero: 1) Crear un cronograma de estudio, 2) Hacer resúmenes y mapas conceptuales, 3) Practicar con exámenes anteriores, 4) Estudiar en grupos. ¿Cuándo es tu próximo examen?';
          } else if (
            lowerMessage.includes('tiempo') ||
            lowerMessage.includes('horario')
          ) {
            response =
              'La gestión del tiempo es clave para el éxito académico. Te ayudo a crear un horario equilibrado que incluya tiempo de estudio, descanso y actividades personales. ¿Cuántas horas al día puedes dedicar al estudio?';
          } else if (
            ...
            response =
              '¡Hola! Me alegra verte por aquí. Estoy aquí para ayudarte a optimizar tu tiempo de estudio y crear planes personalizados. ¿Qué materias estás cursando actualmente?';
          } else {
      ...
      ); // Tiempo aleatorio entre 1.5-2.5 segundos
    },
