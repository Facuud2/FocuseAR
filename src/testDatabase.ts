...
export async function testDatabase() {
  try {
    ...
    // Leer materiales
    const materialsSnap = await getDocs(query(collection(db, 'materials'), where('userId', '==', userData.uid)));
    console.log('📂 Materiales encontrados:', materialsSnap.docs.length);

    // Leer planes de estudio
    const plansSnap = await getDocs(query(collection(db, 'studyPlans'), where('userId', '==', userData.uid)));
